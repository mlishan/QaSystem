                  ...
                  <tbody>
                    ...
                    @foreach($projects as $project)
                      <tr>
                          ...
                          <td><a>{{$project->name}}</a>
                              <br/>
                              <small class="text-muted"><b>Started Date: {{$project->start_date}}</b></small>
                          </td>
                          <td>
                              <ul class="list-inline">
                                  <li class="list-block-item">
                                    @if($project->owner?->image)
                                      <img alt="Avatar" class="table-avatar" src="{{ asset('images/'.$project->owner?->image) }}">
                                    @else
                                        <img src="{{asset('images/avatar5.png')}}" class="table-avatar" alt="User Image">
                                    @endif
                                  </li>
                                  <li class="list-block-item">
                                    {{$project->owner?->name}}
                          ...
                          <td class="project-actions text-right">
                              <a class="btn btn-warning btn-sm" href="{{ url('viewprojects/'.$project->id) }}">
                                  <i class="fas fa-search"></i>
                                  Check On
                              </a>
                              <a class="btn btn-success btn-sm" href="{{ url('editprojects/'.$project->id) }}">
...
<script src="{{asset('js/function/projectSave.js')}}"></script>
@endsection
